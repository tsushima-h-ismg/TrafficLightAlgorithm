...
namespace TrafficLightAlgorithm
{
    public partial class F_TrafficLight : Form
    {
        ...
        private void Btn_Start_Click(object sender, EventArgs e)
        {
            ...
            else if (!IsInterrupt && IsTrafficEnable)
            {
        ...
        private async void LoopTrafficPhase(int phaseNum, List<TrafficPhase> phases)
        {
            ...
            Task cancelTask = Task.Run(() => Cts.Token.WaitHandle.WaitOne());  // キャンセル要求を受け取るタスク
            ...
            while (!CancelRequest)
            {
                for (int i = startPhase; i < phases.Count; i++)
                {
                    ...
                    await Task.WhenAny(Task.Delay(phases[i].WaitMSec), cancelTask);  // phases[i].WaitMSecミリ秒待機する
                }

                startPhase = 0;  // 開始フェーズの番号を0に設定する
            }

            CancelRequest = false;  // キャンセル要求を無効にする
        }
