    ...
    public partial class F_TrafficLight : Form
    {
        ...
        private bool CheckSecText(string checkText, int minValue, int maxValue)
        {
            try
            {
                ...
                if (!int.TryParse(douvalue.ToString(), out int checkValue)) return false;  // double型を変換した文字列がint型に変換できない場合は終了する
        ...
        private int ConvertToInt(string str)
        {
            try
            {
                double.TryParse(str, out double douValue);
                int.TryParse(douValue.ToString(), out int result);
                return result;
            }
            catch
            {
